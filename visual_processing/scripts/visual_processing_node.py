...
def color_detect(img, color):
    ...
    img_h, img_w = img.shape[:2]
    frame_resize = cv2.resize(img_copy, size_m, interpolation=cv2.INTER_NEAREST)
    frame_lab = cv2.cvtColor(frame_resize, cv2.COLOR_BGR2LAB)  # transfer image to LAB space
    ...
    if color in color_range_list:
        ...
        contours = cv2.findContours(dilated, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)[-2]         # find all contours
        area_max_contour, area_max = getAreaMaxContour(contours)                                   # find the largest contours
